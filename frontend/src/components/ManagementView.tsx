...
export function ManagementView({ accessToken, role }: ManagementViewProps) {
  ...
  const [evaluationTrend, setEvaluationTrend] = useState<TrendSummary | null>(null)
  ...
  const completed = (message: string) => { setNotice(message); setDraftOpen(false); setSourceTrend(null); setEvaluationTrend(null); setRefresh((value) => value + 1) }
  ...
  return (
    <main className="management-page">
      ...
      <EvaluationDialog trend={evaluationTrend} accessToken={accessToken} onClose={() => setEvaluationTrend(null)} onCreated={completed} />
    </main>
  )
}
